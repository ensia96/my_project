...
def get_product_data():
    ...
    for product_code in product_codes:
        ...
        button = driver.find_elements_by_xpath(
            '//*[@id="wrapper"]/main/section/div[2]/div/div[4]/div/div[1]/a/span[1]'
        )[0]
        button.click()
        product_name = driver.find_element_by_xpath(
            '//*[@id="wrapper"]/main/section/div[2]/div/div[3]/div[2]/div[1]/h1'
        ).text
        price = driver.find_element_by_xpath(
            '//*[@id="wrapper"]/main/section/div[2]/div/div[3]/div[2]/div[1]/p[1]/span'
        ).text
        price = price.split(" ")[0]
        price = re.sub(pattern="[^\w\s]", repl="", string=price)  # ,제거
        price = int(price)
        gender = driver.find_element_by_xpath(
            '//*[@id="wrapper"]/main/section/div[2]/div/div[3]/div[2]/div[1]/p[2]'
        ).text
        try:
        ...
        color_name = driver.find_element_by_xpath(
            '//*[@id="wrapper"]/main/section/div[2]/div/div[3]/div[2]/form/div[1]/div[1]/div[1]/span[2]'
        ).text
        series_list = []
        series_img_list = []
        series_container = driver.find_element_by_xpath(
            '//*[@id="wrapper"]/main/section/div[2]/div/div[3]/div[2]/form/div[1]/div[2]'
        )
        series = series_container.find_elements_by_class_name("selectable")
        for shoe in series:
            series_list.append(
                shoe.find_element_by_tag_name("a").get_attribute("href").split("/")[-1]
            )
            series_img_list.append(
                shoe.find_element_by_tag_name("img").get_attribute("src")
            )
        product_info = driver.find_element_by_xpath(
            '//*[@id="wrapper"]/main/section/div[2]/div/div[4]/div/div[1]/div'
        )
        info_list = []
        ...
        try:
            image_big = product_info.find_element_by_tag_name("img").get_attribute(
                "src"
            )
        except NoSuchElementException:
            image_big = None
        try:
            size_list = []
            size_container = driver.find_element_by_xpath(
                '//*[@id="wrapper"]/main/section/div[2]/div/div[3]/div[2]/form/div[1]/div[4]/div/div'
            )
        ...
        gallery = driver.find_element_by_xpath(
            '//*[@id="wrapper"]/main/section/div[2]/div/div[2]/div[1]/div[2]/div[1]'
        )
...
def get_instagram_data():
    ...
    driver.get(
        "https://www.attractt.com/embed/grid/CCPPuTmry6XL961?loc=https://www.converse.co.kr/&target=attractt-ifm-0"
    )
    time.sleep(4)
    ...
    for feed in feeds:
        ...
        profile_img = (
            profile.find_element_by_class_name("user")
            .find_element_by_tag_name("img")
            .get_attribute("src")
        )
        username = (
            profile.find_element_by_class_name("info")
            .find_element_by_class_name("username")
            .text
        )
        created_at = (
            profile.find_element_by_class_name("info")
            .find_element_by_class_name("time")
            .text
        )
        writer.writerow(
            [thumbnail.get_attribute("src"), profile_img, username, created_at]
        )


def get_store_info():
    driver.get("https://www.converse.co.kr/store")
    store_infos = driver.find_element_by_xpath(
        '//*[@id="wrapper"]/main/article/div/div[2]/div[2]/div'
    ).get_attribute("data-store-list")
    print(store_infos)
