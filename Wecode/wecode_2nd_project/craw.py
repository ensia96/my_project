...
def for_main(target):
    ...
    for collection in [{
        'name': sou.find_element_by_tag_name('h2').get_attribute('aria-label'),
        'elements': [{
        ...
        if collection['name'] not in os.listdir('./ym_data/collections'):
            ...
            for element in collection['elements']:
                for_list(element['link'],i, collection['name'])
                i += 1
    ...
    for newthings in coll:
        ...
        csv.writer(newcsv).writerow([newthings])
...
def for_list(target, i, col):
    ...
    if 0<len(driver.find_elements_by_css_selector('ytmusic-data-bound-header-renderer')):
        ...
        list_sub    = list_header.find_element_by_class_name('byline')
    ...
    newcsv = open("./playlists.csv",'a',encoding='utf-8')

    if col in ['내 관련 재생목록','최신 음악']:
        if list_header.find_element_by_class_name('title').text == '최신음악 믹스':
            csv.writer(newcsv).writerow([
    ...
    newcsv = open('./'+str(i)+'.csv','w+',encoding='utf-8')
    ...
    for i in itemlist:
        ...
        csv.writer(newcsv).writerow(items)
